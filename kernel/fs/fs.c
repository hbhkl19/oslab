...
void fs_init()
{
    ...
    printf("[fs_test1] read: %s\n", strbuf);
    inode_unlock_free(tmp_2);

    printf("[fs_test1] over\n");
    while (1); 
#elif FS_TEST == 2
    // 目录测试
    inode_t* ip = inode_alloc(INODE_ROOT);    
    ...
    if(blockcmp(t_tmp, t_str) == true)
        printf("[fs_test3] success\n");
    else
        printf("[fs_test3] fail\n");
