...
static void copy_range(pgtbl_t old, pgtbl_t new, uint64 begin, uint64 end)
{
    ...
    for(va = begin; va < end; va += PGSIZE)
    {
        ...
        assert(page != 0, "uvm_copy_pgtbl: pmem_alloc");
...
// 在uvm_munmap里使用
__attribute__((unused)) static void mmap_merge(mmap_region_t* mmap_1, mmap_region_t* mmap_2, bool keep_mmap_1)
{
    // 确保有效和紧临
...
static void destroy_pgtbl(pgtbl_t pgtbl, uint32 level, uint64 base)
{
    for(int i = 0; i < PGSIZE / sizeof(pte_t); i++) {
        pte_t pte = pgtbl[i];
        if(!(pte & PTE_V)) continue;

        uint64 va = base + ((uint64)i << VA_SHIFT(level));
        if(PTE_CHECK(pte)) {
            destroy_pgtbl((pgtbl_t)PTE_TO_PA(pte), level - 1, va);
            pmem_free((uint64)PTE_TO_PA(pte), true);
        } else {
...
{
    destroy_pgtbl(pgtbl, 2, 0);
    // 根页表在 proc_pgtbl_init 中从用户池申请
...
void uvm_copy_pgtbl(pgtbl_t old, pgtbl_t new, uint64 heap_top, uint32 ustack_pages, mmap_region_t* mmap)
{
    ...
    uint64 heap_end = PG_ROUND_UP(heap_top);
    if(heap_end > 0) {
        copy_range(old, new, 0, heap_end);
    }

    /* step-2: ustack */
    for(uint32 i = 0; i < ustack_pages; i++) {
        uint64 va = TRAPFRAME - (i + 1) * PGSIZE;
        copy_range(old, new, va, va + PGSIZE);
    }
...
{
    while(len > 0) {
        uint64 va0 = PG_ROUND_DOWN(src);
        pte_t* pte = vm_getpte(pgtbl, va0, false);
        assert(pte != NULL && (*pte & PTE_V) && (*pte & PTE_U), "uvm_copyin: invalid pte");

        uint64 pa = (uint64)PTE_TO_PA(*pte);
        uint64 n = PGSIZE - (src - va0);
        if(n > len) n = len;

        memmove((void*)dst, (void*)(pa + (src - va0)), n);
        len -= n;
        dst += n;
        src += n;
    }
...
{
    while(len > 0) {
        uint64 va0 = PG_ROUND_DOWN(dst);
        pte_t* pte = vm_getpte(pgtbl, va0, false);
        assert(pte != NULL && (*pte & PTE_V) && (*pte & PTE_U), "uvm_copyout: invalid pte");

        uint64 pa = (uint64)PTE_TO_PA(*pte);
        uint64 n = PGSIZE - (dst - va0);
        if(n > len) n = len;

        memmove((void*)(pa + (dst - va0)), (void*)src, n);
        len -= n;
        dst += n;
        src += n;
    }
...
void uvm_copyin_str(pgtbl_t pgtbl, uint64 dst, uint64 src, uint32 maxlen)
{
    ...
    while(copied < maxlen) {
        uint64 va0 = PG_ROUND_DOWN(src);
        pte_t* pte = vm_getpte(pgtbl, va0, false);
        assert(pte != NULL && (*pte & PTE_V) && (*pte & PTE_U), "uvm_copyin_str: invalid pte");

        uint64 pa = (uint64)PTE_TO_PA(*pte);
        uint64 n = PGSIZE - (src - va0);
        if(n > maxlen - copied) n = maxlen - copied;

        char* psrc = (char*)(pa + (src - va0));
