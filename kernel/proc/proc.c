...
// 全局的pid和保护它的锁 
// 从 2 开始，因为 pid=1 留给 init 进程（在 proc_make_first 中手动设置）
...
static void fork_return()
{
    // 由于调度器中上了锁，所以这里需要解锁
    proc_t* p = myproc();
    DEBUG_LOG("[fork_return] pid=%d epc=0x%lx sp=0x%lx\n", p->pid, p->tf->epc, p->tf->sp);
...
void proc_init()
{
    global_pid = 2;  // 从 2 开始，pid=1 留给 init 进程
...
#endif
    DEBUG_LOG("[fork] parent=%d child=%d child_epc=0x%lx\n", parent->pid, pid, child->tf->epc);
...
int proc_clone(uint64 stack)
{
    ...
    *(child->tf) = *(parent->tf);
    ...
    int pid = child->pid;
#ifdef PROC_DEBUG
    printf("[proc] clone: parent=%d child=%d stack=0x%lx\n", parent->pid, pid, stack);
#endif
    DEBUG_LOG("[clone] parent=%d child=%d stack=0x%lx child_epc=0x%lx child_sp=0x%lx\n", 
              parent->pid, pid, stack, child->tf->epc, child->tf->sp);
    spinlock_release(&child->lk);
    spinlock_release(&parent->lk);
    return pid;
}
...
{
    proc_t* p = myproc();
    // init/proczero退出时直接关机
    if(p == proczero) {
        DEBUG_LOG("[poweroff] init exited, shutting down via SBI\n");
        sbi_shutdown();
    }
...
// 唤醒所有计时器到期的进程 (被时钟中断调用)
void wakeup_timer_sleepers(uint64 current_ticks)
{
    for(int i = 0; i < NPROC; i++) {
        proc_t* p = &procs[i];
        spinlock_acquire(&p->lk);
        if(p->state == SLEEPING && p->sleep_until > 0 && current_ticks >= p->sleep_until) {
            p->state = RUNNABLE;
        }
        spinlock_release(&p->lk);
    }
...
void proc_make_first()
{
    ...
    // pid 设置 (从1开始，符合Unix惯例)
    p->pid = 1;
    p->parent = p;  // init进程的父进程是自己
    ...
    f_stderr->type = FD_DEVICE;
    f_stderr->readable = false;
    f_stderr->writable = true;
