
/home/hbh/oslab/oslab/user/build/riscv64/fork:     file format elf64-littleriscv


Disassembly of section .text:

0000000000001000 <_start>:
.section .text.entry
.globl _start
_start:
    mv a0, sp
    1000:	850a                	mv	a0,sp
    tail __start_main
    1002:	a05d                	j	10a8 <__start_main>

0000000000001004 <test_fork>:
 * 成功测试时子进程的输出：
 * "  child process."
 */
static int fd[2];

void test_fork(void){
    1004:	1101                	add	sp,sp,-32
    TEST_START(__func__);
    1006:	00001517          	auipc	a0,0x1
    100a:	eda50513          	add	a0,a0,-294 # 1ee0 <__clone+0x2a>
void test_fork(void){
    100e:	ec06                	sd	ra,24(sp)
    TEST_START(__func__);
    1010:	2f0000ef          	jal	1300 <puts>
    1014:	00001517          	auipc	a0,0x1
    1018:	f7c50513          	add	a0,a0,-132 # 1f90 <__func__.0>
    101c:	2e4000ef          	jal	1300 <puts>
    1020:	00001517          	auipc	a0,0x1
    1024:	ed850513          	add	a0,a0,-296 # 1ef8 <__clone+0x42>
    1028:	2d8000ef          	jal	1300 <puts>
    int cpid, wstatus;
    cpid = fork();
    102c:	45b000ef          	jal	1c86 <fork>
    assert(cpid != -1);
    1030:	57fd                	li	a5,-1
    1032:	04f50363          	beq	a0,a5,1078 <test_fork+0x74>

    if(cpid > 0){
    1036:	04a05763          	blez	a0,1084 <test_fork+0x80>
	wait(&wstatus);
    103a:	0068                	add	a0,sp,12
    103c:	547000ef          	jal	1d82 <wait>
	printf("  parent process. wstatus:%d\n", wstatus);
    1040:	45b2                	lw	a1,12(sp)
    1042:	00001517          	auipc	a0,0x1
    1046:	ee650513          	add	a0,a0,-282 # 1f28 <__clone+0x72>
    104a:	2d8000ef          	jal	1322 <printf>
    }else{
	printf("  child process.\n");
	exit(0);
    }
    TEST_END(__func__);
    104e:	00001517          	auipc	a0,0x1
    1052:	f1250513          	add	a0,a0,-238 # 1f60 <__clone+0xaa>
    1056:	2aa000ef          	jal	1300 <puts>
    105a:	00001517          	auipc	a0,0x1
    105e:	f3650513          	add	a0,a0,-202 # 1f90 <__func__.0>
    1062:	29e000ef          	jal	1300 <puts>
    1066:	00001517          	auipc	a0,0x1
    106a:	e9250513          	add	a0,a0,-366 # 1ef8 <__clone+0x42>
    106e:	292000ef          	jal	1300 <puts>
}
    1072:	60e2                	ld	ra,24(sp)
    1074:	6105                	add	sp,sp,32
    1076:	8082                	ret
    assert(cpid != -1);
    1078:	00001517          	auipc	a0,0x1
    107c:	e9050513          	add	a0,a0,-368 # 1f08 <__clone+0x52>
    1080:	51c000ef          	jal	159c <panic>
	printf("  child process.\n");
    1084:	00001517          	auipc	a0,0x1
    1088:	ec450513          	add	a0,a0,-316 # 1f48 <__clone+0x92>
    108c:	296000ef          	jal	1322 <printf>
	exit(0);
    1090:	4501                	li	a0,0
    1092:	421000ef          	jal	1cb2 <exit>
    1096:	bf65                	j	104e <test_fork+0x4a>

0000000000001098 <main>:

int main(void){
    1098:	1141                	add	sp,sp,-16
    109a:	e406                	sd	ra,8(sp)
    test_fork();
    109c:	f69ff0ef          	jal	1004 <test_fork>
    return 0;
}
    10a0:	60a2                	ld	ra,8(sp)
    10a2:	4501                	li	a0,0
    10a4:	0141                	add	sp,sp,16
    10a6:	8082                	ret

00000000000010a8 <__start_main>:
#include <unistd.h>

extern int main();

int __start_main(long *p)
{
    10a8:	85aa                	mv	a1,a0
	int argc = p[0];
	char **argv = (void *)(p+1);

	exit(main(argc, argv));
    10aa:	4108                	lw	a0,0(a0)
{
    10ac:	1141                	add	sp,sp,-16
	exit(main(argc, argv));
    10ae:	05a1                	add	a1,a1,8
{
    10b0:	e406                	sd	ra,8(sp)
	exit(main(argc, argv));
    10b2:	fe7ff0ef          	jal	1098 <main>
    10b6:	3fd000ef          	jal	1cb2 <exit>
	return 0;
}
    10ba:	60a2                	ld	ra,8(sp)
    10bc:	4501                	li	a0,0
    10be:	0141                	add	sp,sp,16
    10c0:	8082                	ret

00000000000010c2 <printint.constprop.0>:
    write(f, s, l);
}

static char digits[] = "0123456789abcdef";

static void printint(int xx, int base, int sign)
    10c2:	7179                	add	sp,sp,-48
    10c4:	f406                	sd	ra,40(sp)
{
    char buf[16 + 1];
    int i;
    uint x;

    if (sign && (sign = xx < 0))
    10c6:	12054863          	bltz	a0,11f6 <printint.constprop.0+0x134>

    buf[16] = 0;
    i = 15;
    do
    {
        buf[i--] = digits[x % base];
    10ca:	02b577bb          	remuw	a5,a0,a1
    10ce:	00001697          	auipc	a3,0x1
    10d2:	ed268693          	add	a3,a3,-302 # 1fa0 <digits>
    buf[16] = 0;
    10d6:	00010c23          	sb	zero,24(sp)
        buf[i--] = digits[x % base];
    10da:	0005871b          	sext.w	a4,a1
    10de:	1782                	sll	a5,a5,0x20
    10e0:	9381                	srl	a5,a5,0x20
    10e2:	97b6                	add	a5,a5,a3
    10e4:	0007c783          	lbu	a5,0(a5)
    } while ((x /= base) != 0);
    10e8:	02b5583b          	divuw	a6,a0,a1
        buf[i--] = digits[x % base];
    10ec:	00f10ba3          	sb	a5,23(sp)
    } while ((x /= base) != 0);
    10f0:	1ab56663          	bltu	a0,a1,129c <printint.constprop.0+0x1da>
        buf[i--] = digits[x % base];
    10f4:	02e8763b          	remuw	a2,a6,a4
    10f8:	1602                	sll	a2,a2,0x20
    10fa:	9201                	srl	a2,a2,0x20
    10fc:	9636                	add	a2,a2,a3
    10fe:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    1102:	02e855bb          	divuw	a1,a6,a4
        buf[i--] = digits[x % base];
    1106:	00c10b23          	sb	a2,22(sp)
    } while ((x /= base) != 0);
    110a:	12e86c63          	bltu	a6,a4,1242 <printint.constprop.0+0x180>
        buf[i--] = digits[x % base];
    110e:	02e5f63b          	remuw	a2,a1,a4
    1112:	1602                	sll	a2,a2,0x20
    1114:	9201                	srl	a2,a2,0x20
    1116:	9636                	add	a2,a2,a3
    1118:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    111c:	02e5d83b          	divuw	a6,a1,a4
        buf[i--] = digits[x % base];
    1120:	00c10aa3          	sb	a2,21(sp)
    } while ((x /= base) != 0);
    1124:	12e5e863          	bltu	a1,a4,1254 <printint.constprop.0+0x192>
        buf[i--] = digits[x % base];
    1128:	02e8763b          	remuw	a2,a6,a4
    112c:	1602                	sll	a2,a2,0x20
    112e:	9201                	srl	a2,a2,0x20
    1130:	9636                	add	a2,a2,a3
    1132:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    1136:	02e855bb          	divuw	a1,a6,a4
        buf[i--] = digits[x % base];
    113a:	00c10a23          	sb	a2,20(sp)
    } while ((x /= base) != 0);
    113e:	12e86463          	bltu	a6,a4,1266 <printint.constprop.0+0x1a4>
        buf[i--] = digits[x % base];
    1142:	02e5f63b          	remuw	a2,a1,a4
    1146:	1602                	sll	a2,a2,0x20
    1148:	9201                	srl	a2,a2,0x20
    114a:	9636                	add	a2,a2,a3
    114c:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    1150:	02e5d83b          	divuw	a6,a1,a4
        buf[i--] = digits[x % base];
    1154:	00c109a3          	sb	a2,19(sp)
    } while ((x /= base) != 0);
    1158:	12e5e063          	bltu	a1,a4,1278 <printint.constprop.0+0x1b6>
        buf[i--] = digits[x % base];
    115c:	02e8763b          	remuw	a2,a6,a4
    1160:	1602                	sll	a2,a2,0x20
    1162:	9201                	srl	a2,a2,0x20
    1164:	9636                	add	a2,a2,a3
    1166:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    116a:	02e855bb          	divuw	a1,a6,a4
        buf[i--] = digits[x % base];
    116e:	00c10923          	sb	a2,18(sp)
    } while ((x /= base) != 0);
    1172:	0ae86f63          	bltu	a6,a4,1230 <printint.constprop.0+0x16e>
        buf[i--] = digits[x % base];
    1176:	02e5f63b          	remuw	a2,a1,a4
    117a:	1602                	sll	a2,a2,0x20
    117c:	9201                	srl	a2,a2,0x20
    117e:	9636                	add	a2,a2,a3
    1180:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    1184:	02e5d83b          	divuw	a6,a1,a4
        buf[i--] = digits[x % base];
    1188:	00c108a3          	sb	a2,17(sp)
    } while ((x /= base) != 0);
    118c:	0ee5ef63          	bltu	a1,a4,128a <printint.constprop.0+0x1c8>
        buf[i--] = digits[x % base];
    1190:	02e8763b          	remuw	a2,a6,a4
    1194:	1602                	sll	a2,a2,0x20
    1196:	9201                	srl	a2,a2,0x20
    1198:	9636                	add	a2,a2,a3
    119a:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    119e:	02e855bb          	divuw	a1,a6,a4
        buf[i--] = digits[x % base];
    11a2:	00c10823          	sb	a2,16(sp)
    } while ((x /= base) != 0);
    11a6:	0ee86d63          	bltu	a6,a4,12a0 <printint.constprop.0+0x1de>
        buf[i--] = digits[x % base];
    11aa:	02e5f63b          	remuw	a2,a1,a4
    11ae:	1602                	sll	a2,a2,0x20
    11b0:	9201                	srl	a2,a2,0x20
    11b2:	9636                	add	a2,a2,a3
    11b4:	00064603          	lbu	a2,0(a2)
    } while ((x /= base) != 0);
    11b8:	02e5d7bb          	divuw	a5,a1,a4
        buf[i--] = digits[x % base];
    11bc:	00c107a3          	sb	a2,15(sp)
    } while ((x /= base) != 0);
    11c0:	0ee5e963          	bltu	a1,a4,12b2 <printint.constprop.0+0x1f0>
        buf[i--] = digits[x % base];
    11c4:	1782                	sll	a5,a5,0x20
    11c6:	9381                	srl	a5,a5,0x20
    11c8:	96be                	add	a3,a3,a5
    11ca:	0006c783          	lbu	a5,0(a3)
    11ce:	4599                	li	a1,6
    11d0:	00f10723          	sb	a5,14(sp)

    if (sign)
    11d4:	00055763          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    11d8:	02d00793          	li	a5,45
    11dc:	00f106a3          	sb	a5,13(sp)
        buf[i--] = digits[x % base];
    11e0:	4595                	li	a1,5
    write(f, s, l);
    11e2:	003c                	add	a5,sp,8
    11e4:	4641                	li	a2,16
    11e6:	9e0d                	subw	a2,a2,a1
    11e8:	4505                	li	a0,1
    11ea:	95be                	add	a1,a1,a5
    11ec:	26d000ef          	jal	1c58 <write>
    i++;
    if (i < 0)
        puts("printint error");
    out(stdout, buf + i, 16 - i);
}
    11f0:	70a2                	ld	ra,40(sp)
    11f2:	6145                	add	sp,sp,48
    11f4:	8082                	ret
        x = -xx;
    11f6:	40a0063b          	negw	a2,a0
        buf[i--] = digits[x % base];
    11fa:	02b677bb          	remuw	a5,a2,a1
    11fe:	00001697          	auipc	a3,0x1
    1202:	da268693          	add	a3,a3,-606 # 1fa0 <digits>
    buf[16] = 0;
    1206:	00010c23          	sb	zero,24(sp)
        buf[i--] = digits[x % base];
    120a:	0005871b          	sext.w	a4,a1
    120e:	1782                	sll	a5,a5,0x20
    1210:	9381                	srl	a5,a5,0x20
    1212:	97b6                	add	a5,a5,a3
    1214:	0007c783          	lbu	a5,0(a5)
    } while ((x /= base) != 0);
    1218:	02b6583b          	divuw	a6,a2,a1
        buf[i--] = digits[x % base];
    121c:	00f10ba3          	sb	a5,23(sp)
    } while ((x /= base) != 0);
    1220:	ecb67ae3          	bgeu	a2,a1,10f4 <printint.constprop.0+0x32>
        buf[i--] = '-';
    1224:	02d00793          	li	a5,45
    1228:	00f10b23          	sb	a5,22(sp)
        buf[i--] = digits[x % base];
    122c:	45b9                	li	a1,14
    122e:	bf55                	j	11e2 <printint.constprop.0+0x120>
    1230:	45a9                	li	a1,10
    if (sign)
    1232:	fa0558e3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    1236:	02d00793          	li	a5,45
    123a:	00f108a3          	sb	a5,17(sp)
        buf[i--] = digits[x % base];
    123e:	45a5                	li	a1,9
    1240:	b74d                	j	11e2 <printint.constprop.0+0x120>
    1242:	45b9                	li	a1,14
    if (sign)
    1244:	f8055fe3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    1248:	02d00793          	li	a5,45
    124c:	00f10aa3          	sb	a5,21(sp)
        buf[i--] = digits[x % base];
    1250:	45b5                	li	a1,13
    1252:	bf41                	j	11e2 <printint.constprop.0+0x120>
    1254:	45b5                	li	a1,13
    if (sign)
    1256:	f80556e3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    125a:	02d00793          	li	a5,45
    125e:	00f10a23          	sb	a5,20(sp)
        buf[i--] = digits[x % base];
    1262:	45b1                	li	a1,12
    1264:	bfbd                	j	11e2 <printint.constprop.0+0x120>
    1266:	45b1                	li	a1,12
    if (sign)
    1268:	f6055de3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    126c:	02d00793          	li	a5,45
    1270:	00f109a3          	sb	a5,19(sp)
        buf[i--] = digits[x % base];
    1274:	45ad                	li	a1,11
    1276:	b7b5                	j	11e2 <printint.constprop.0+0x120>
    1278:	45ad                	li	a1,11
    if (sign)
    127a:	f60554e3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    127e:	02d00793          	li	a5,45
    1282:	00f10923          	sb	a5,18(sp)
        buf[i--] = digits[x % base];
    1286:	45a9                	li	a1,10
    1288:	bfa9                	j	11e2 <printint.constprop.0+0x120>
    128a:	45a5                	li	a1,9
    if (sign)
    128c:	f4055be3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    1290:	02d00793          	li	a5,45
    1294:	00f10823          	sb	a5,16(sp)
        buf[i--] = digits[x % base];
    1298:	45a1                	li	a1,8
    129a:	b7a1                	j	11e2 <printint.constprop.0+0x120>
    i = 15;
    129c:	45bd                	li	a1,15
    129e:	b791                	j	11e2 <printint.constprop.0+0x120>
        buf[i--] = digits[x % base];
    12a0:	45a1                	li	a1,8
    if (sign)
    12a2:	f40550e3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    12a6:	02d00793          	li	a5,45
    12aa:	00f107a3          	sb	a5,15(sp)
        buf[i--] = digits[x % base];
    12ae:	459d                	li	a1,7
    12b0:	bf0d                	j	11e2 <printint.constprop.0+0x120>
    12b2:	459d                	li	a1,7
    if (sign)
    12b4:	f20557e3          	bgez	a0,11e2 <printint.constprop.0+0x120>
        buf[i--] = '-';
    12b8:	02d00793          	li	a5,45
    12bc:	00f10723          	sb	a5,14(sp)
        buf[i--] = digits[x % base];
    12c0:	4599                	li	a1,6
    12c2:	b705                	j	11e2 <printint.constprop.0+0x120>

00000000000012c4 <getchar>:
{
    12c4:	1101                	add	sp,sp,-32
    read(stdin, &byte, 1);
    12c6:	00f10593          	add	a1,sp,15
    12ca:	4605                	li	a2,1
    12cc:	4501                	li	a0,0
{
    12ce:	ec06                	sd	ra,24(sp)
    char byte = 0;
    12d0:	000107a3          	sb	zero,15(sp)
    read(stdin, &byte, 1);
    12d4:	17b000ef          	jal	1c4e <read>
}
    12d8:	60e2                	ld	ra,24(sp)
    12da:	00f14503          	lbu	a0,15(sp)
    12de:	6105                	add	sp,sp,32
    12e0:	8082                	ret

00000000000012e2 <putchar>:
{
    12e2:	1101                	add	sp,sp,-32
    12e4:	87aa                	mv	a5,a0
    return write(stdout, &byte, 1);
    12e6:	00f10593          	add	a1,sp,15
    12ea:	4605                	li	a2,1
    12ec:	4505                	li	a0,1
{
    12ee:	ec06                	sd	ra,24(sp)
    char byte = c;
    12f0:	00f107a3          	sb	a5,15(sp)
    return write(stdout, &byte, 1);
    12f4:	165000ef          	jal	1c58 <write>
}
    12f8:	60e2                	ld	ra,24(sp)
    12fa:	2501                	sext.w	a0,a0
    12fc:	6105                	add	sp,sp,32
    12fe:	8082                	ret

0000000000001300 <puts>:
{
    1300:	1141                	add	sp,sp,-16
    1302:	e406                	sd	ra,8(sp)
    1304:	e022                	sd	s0,0(sp)
    1306:	842a                	mv	s0,a0
    r = -(write(stdout, s, strlen(s)) < 0);
    1308:	574000ef          	jal	187c <strlen>
    130c:	862a                	mv	a2,a0
    130e:	85a2                	mv	a1,s0
    1310:	4505                	li	a0,1
    1312:	147000ef          	jal	1c58 <write>
}
    1316:	60a2                	ld	ra,8(sp)
    1318:	6402                	ld	s0,0(sp)
    r = -(write(stdout, s, strlen(s)) < 0);
    131a:	957d                	sra	a0,a0,0x3f
    return r;
    131c:	2501                	sext.w	a0,a0
}
    131e:	0141                	add	sp,sp,16
    1320:	8082                	ret

0000000000001322 <printf>:
    out(stdout, buf, i);
}

// Print to the console. only understands %d, %x, %p, %s.
void printf(const char *fmt, ...)
{
    1322:	7171                	add	sp,sp,-176
    1324:	f85a                	sd	s6,48(sp)
    1326:	ed3e                	sd	a5,152(sp)
    buf[i++] = '0';
    1328:	7b61                	lui	s6,0xffff8
    va_list ap;
    int cnt = 0, l = 0;
    char *a, *z, *s = (char *)fmt, str;
    int f = stdout;

    va_start(ap, fmt);
    132a:	18bc                	add	a5,sp,120
{
    132c:	e8ca                	sd	s2,80(sp)
    132e:	e4ce                	sd	s3,72(sp)
    1330:	e0d2                	sd	s4,64(sp)
    1332:	fc56                	sd	s5,56(sp)
    1334:	f486                	sd	ra,104(sp)
    1336:	f0a2                	sd	s0,96(sp)
    1338:	eca6                	sd	s1,88(sp)
    133a:	fcae                	sd	a1,120(sp)
    133c:	e132                	sd	a2,128(sp)
    133e:	e536                	sd	a3,136(sp)
    1340:	e93a                	sd	a4,144(sp)
    1342:	f142                	sd	a6,160(sp)
    1344:	f546                	sd	a7,168(sp)
    va_start(ap, fmt);
    1346:	e03e                	sd	a5,0(sp)
    for (;;)
    {
        if (!*s)
            break;
        for (a = s; *s && *s != '%'; s++)
    1348:	02500913          	li	s2,37
        out(f, a, l);
        if (l)
            continue;
        if (s[1] == 0)
            break;
        switch (s[1])
    134c:	07300a13          	li	s4,115
    1350:	07800a93          	li	s5,120
    buf[i++] = '0';
    1354:	830b4b13          	xor	s6,s6,-2000
        buf[i++] = digits[x >> (sizeof(uint64) * 8 - 4)];
    1358:	00001997          	auipc	s3,0x1
    135c:	c4898993          	add	s3,s3,-952 # 1fa0 <digits>
        if (!*s)
    1360:	00054783          	lbu	a5,0(a0)
    1364:	16078a63          	beqz	a5,14d8 <printf+0x1b6>
    1368:	862a                	mv	a2,a0
        for (a = s; *s && *s != '%'; s++)
    136a:	19278d63          	beq	a5,s2,1504 <printf+0x1e2>
    136e:	00164783          	lbu	a5,1(a2)
    1372:	0605                	add	a2,a2,1
    1374:	fbfd                	bnez	a5,136a <printf+0x48>
    1376:	84b2                	mv	s1,a2
        l = z - a;
    1378:	40a6043b          	subw	s0,a2,a0
    write(f, s, l);
    137c:	85aa                	mv	a1,a0
    137e:	8622                	mv	a2,s0
    1380:	4505                	li	a0,1
    1382:	0d7000ef          	jal	1c58 <write>
        if (l)
    1386:	1a041463          	bnez	s0,152e <printf+0x20c>
        if (s[1] == 0)
    138a:	0014c783          	lbu	a5,1(s1)
    138e:	14078563          	beqz	a5,14d8 <printf+0x1b6>
        switch (s[1])
    1392:	1b478063          	beq	a5,s4,1532 <printf+0x210>
    1396:	14fa6b63          	bltu	s4,a5,14ec <printf+0x1ca>
    139a:	06400713          	li	a4,100
    139e:	1ee78063          	beq	a5,a4,157e <printf+0x25c>
    13a2:	07000713          	li	a4,112
    13a6:	1ae79963          	bne	a5,a4,1558 <printf+0x236>
            break;
        case 'x':
            printint(va_arg(ap, int), 16, 1);
            break;
        case 'p':
            printptr(va_arg(ap, uint64));
    13aa:	6702                	ld	a4,0(sp)
    buf[i++] = '0';
    13ac:	01611423          	sh	s6,8(sp)
    write(f, s, l);
    13b0:	4649                	li	a2,18
            printptr(va_arg(ap, uint64));
    13b2:	631c                	ld	a5,0(a4)
    13b4:	0721                	add	a4,a4,8
    13b6:	e03a                	sd	a4,0(sp)
    for (j = 0; j < (sizeof(uint64) * 2); j++, x <<= 4)
    13b8:	00479293          	sll	t0,a5,0x4
    13bc:	00879f93          	sll	t6,a5,0x8
    13c0:	00c79f13          	sll	t5,a5,0xc
    13c4:	01079e93          	sll	t4,a5,0x10
    13c8:	01479e13          	sll	t3,a5,0x14
    13cc:	01879313          	sll	t1,a5,0x18
    13d0:	01c79893          	sll	a7,a5,0x1c
    13d4:	02479813          	sll	a6,a5,0x24
    13d8:	02879513          	sll	a0,a5,0x28
    13dc:	02c79593          	sll	a1,a5,0x2c
    13e0:	03079693          	sll	a3,a5,0x30
    13e4:	03479713          	sll	a4,a5,0x34
        buf[i++] = digits[x >> (sizeof(uint64) * 8 - 4)];
    13e8:	03c7d413          	srl	s0,a5,0x3c
    13ec:	01c7d39b          	srlw	t2,a5,0x1c
    13f0:	03c2d293          	srl	t0,t0,0x3c
    13f4:	03cfdf93          	srl	t6,t6,0x3c
    13f8:	03cf5f13          	srl	t5,t5,0x3c
    13fc:	03cede93          	srl	t4,t4,0x3c
    1400:	03ce5e13          	srl	t3,t3,0x3c
    1404:	03c35313          	srl	t1,t1,0x3c
    1408:	03c8d893          	srl	a7,a7,0x3c
    140c:	03c85813          	srl	a6,a6,0x3c
    1410:	9171                	srl	a0,a0,0x3c
    1412:	91f1                	srl	a1,a1,0x3c
    1414:	92f1                	srl	a3,a3,0x3c
    1416:	9371                	srl	a4,a4,0x3c
    1418:	96ce                	add	a3,a3,s3
    141a:	974e                	add	a4,a4,s3
    141c:	944e                	add	s0,s0,s3
    141e:	92ce                	add	t0,t0,s3
    1420:	9fce                	add	t6,t6,s3
    1422:	9f4e                	add	t5,t5,s3
    1424:	9ece                	add	t4,t4,s3
    1426:	9e4e                	add	t3,t3,s3
    1428:	934e                	add	t1,t1,s3
    142a:	98ce                	add	a7,a7,s3
    142c:	93ce                	add	t2,t2,s3
    142e:	984e                	add	a6,a6,s3
    1430:	954e                	add	a0,a0,s3
    1432:	95ce                	add	a1,a1,s3
    1434:	0006c083          	lbu	ra,0(a3)
    1438:	0002c283          	lbu	t0,0(t0)
    143c:	00074683          	lbu	a3,0(a4)
    1440:	000fcf83          	lbu	t6,0(t6)
    1444:	000f4f03          	lbu	t5,0(t5)
    1448:	000ece83          	lbu	t4,0(t4)
    144c:	000e4e03          	lbu	t3,0(t3)
    1450:	00034303          	lbu	t1,0(t1)
    1454:	0008c883          	lbu	a7,0(a7)
    1458:	0003c383          	lbu	t2,0(t2)
    145c:	00084803          	lbu	a6,0(a6)
    1460:	00054503          	lbu	a0,0(a0)
    1464:	0005c583          	lbu	a1,0(a1)
    1468:	00044403          	lbu	s0,0(s0)
    for (j = 0; j < (sizeof(uint64) * 2); j++, x <<= 4)
    146c:	03879713          	sll	a4,a5,0x38
        buf[i++] = digits[x >> (sizeof(uint64) * 8 - 4)];
    1470:	9371                	srl	a4,a4,0x3c
    1472:	8bbd                	and	a5,a5,15
    1474:	974e                	add	a4,a4,s3
    1476:	97ce                	add	a5,a5,s3
    1478:	005105a3          	sb	t0,11(sp)
    147c:	01f10623          	sb	t6,12(sp)
    1480:	01e106a3          	sb	t5,13(sp)
    1484:	01d10723          	sb	t4,14(sp)
    1488:	01c107a3          	sb	t3,15(sp)
    148c:	00610823          	sb	t1,16(sp)
    1490:	011108a3          	sb	a7,17(sp)
    1494:	00710923          	sb	t2,18(sp)
    1498:	010109a3          	sb	a6,19(sp)
    149c:	00a10a23          	sb	a0,20(sp)
    14a0:	00b10aa3          	sb	a1,21(sp)
    14a4:	00110b23          	sb	ra,22(sp)
    14a8:	00d10ba3          	sb	a3,23(sp)
    14ac:	00810523          	sb	s0,10(sp)
    14b0:	00074703          	lbu	a4,0(a4)
    14b4:	0007c783          	lbu	a5,0(a5)
    write(f, s, l);
    14b8:	002c                	add	a1,sp,8
    14ba:	4505                	li	a0,1
        buf[i++] = digits[x >> (sizeof(uint64) * 8 - 4)];
    14bc:	00e10c23          	sb	a4,24(sp)
    14c0:	00f10ca3          	sb	a5,25(sp)
    buf[i] = 0;
    14c4:	00010d23          	sb	zero,26(sp)
    write(f, s, l);
    14c8:	790000ef          	jal	1c58 <write>
            // Print unknown % sequence to draw attention.
            putchar('%');
            putchar(s[1]);
            break;
        }
        s += 2;
    14cc:	00248513          	add	a0,s1,2
        if (!*s)
    14d0:	00054783          	lbu	a5,0(a0)
    14d4:	e8079ae3          	bnez	a5,1368 <printf+0x46>
    }
    va_end(ap);
}
    14d8:	70a6                	ld	ra,104(sp)
    14da:	7406                	ld	s0,96(sp)
    14dc:	64e6                	ld	s1,88(sp)
    14de:	6946                	ld	s2,80(sp)
    14e0:	69a6                	ld	s3,72(sp)
    14e2:	6a06                	ld	s4,64(sp)
    14e4:	7ae2                	ld	s5,56(sp)
    14e6:	7b42                	ld	s6,48(sp)
    14e8:	614d                	add	sp,sp,176
    14ea:	8082                	ret
        switch (s[1])
    14ec:	07579663          	bne	a5,s5,1558 <printf+0x236>
            printint(va_arg(ap, int), 16, 1);
    14f0:	6782                	ld	a5,0(sp)
    14f2:	45c1                	li	a1,16
    14f4:	4388                	lw	a0,0(a5)
    14f6:	07a1                	add	a5,a5,8
    14f8:	e03e                	sd	a5,0(sp)
    14fa:	bc9ff0ef          	jal	10c2 <printint.constprop.0>
        s += 2;
    14fe:	00248513          	add	a0,s1,2
    1502:	b7f9                	j	14d0 <printf+0x1ae>
    1504:	84b2                	mv	s1,a2
    1506:	a039                	j	1514 <printf+0x1f2>
        for (z = s; s[0] == '%' && s[1] == '%'; z++, s += 2)
    1508:	0024c783          	lbu	a5,2(s1)
    150c:	0605                	add	a2,a2,1
    150e:	0489                	add	s1,s1,2
    1510:	e72794e3          	bne	a5,s2,1378 <printf+0x56>
    1514:	0014c783          	lbu	a5,1(s1)
    1518:	ff2788e3          	beq	a5,s2,1508 <printf+0x1e6>
        l = z - a;
    151c:	40a6043b          	subw	s0,a2,a0
    write(f, s, l);
    1520:	85aa                	mv	a1,a0
    1522:	8622                	mv	a2,s0
    1524:	4505                	li	a0,1
    1526:	732000ef          	jal	1c58 <write>
        if (l)
    152a:	e60400e3          	beqz	s0,138a <printf+0x68>
    152e:	8526                	mv	a0,s1
    1530:	bd05                	j	1360 <printf+0x3e>
            if ((a = va_arg(ap, char *)) == 0)
    1532:	6782                	ld	a5,0(sp)
    1534:	6380                	ld	s0,0(a5)
    1536:	07a1                	add	a5,a5,8
    1538:	e03e                	sd	a5,0(sp)
    153a:	cc21                	beqz	s0,1592 <printf+0x270>
            l = strnlen(a, 200);
    153c:	0c800593          	li	a1,200
    1540:	8522                	mv	a0,s0
    1542:	424000ef          	jal	1966 <strnlen>
    write(f, s, l);
    1546:	0005061b          	sext.w	a2,a0
    154a:	85a2                	mv	a1,s0
    154c:	4505                	li	a0,1
    154e:	70a000ef          	jal	1c58 <write>
        s += 2;
    1552:	00248513          	add	a0,s1,2
    1556:	bfad                	j	14d0 <printf+0x1ae>
    return write(stdout, &byte, 1);
    1558:	4605                	li	a2,1
    155a:	002c                	add	a1,sp,8
    155c:	4505                	li	a0,1
    char byte = c;
    155e:	01210423          	sb	s2,8(sp)
    return write(stdout, &byte, 1);
    1562:	6f6000ef          	jal	1c58 <write>
    char byte = c;
    1566:	0014c783          	lbu	a5,1(s1)
    return write(stdout, &byte, 1);
    156a:	4605                	li	a2,1
    156c:	002c                	add	a1,sp,8
    156e:	4505                	li	a0,1
    char byte = c;
    1570:	00f10423          	sb	a5,8(sp)
    return write(stdout, &byte, 1);
    1574:	6e4000ef          	jal	1c58 <write>
        s += 2;
    1578:	00248513          	add	a0,s1,2
    157c:	bf91                	j	14d0 <printf+0x1ae>
            printint(va_arg(ap, int), 10, 1);
    157e:	6782                	ld	a5,0(sp)
    1580:	45a9                	li	a1,10
    1582:	4388                	lw	a0,0(a5)
    1584:	07a1                	add	a5,a5,8
    1586:	e03e                	sd	a5,0(sp)
    1588:	b3bff0ef          	jal	10c2 <printint.constprop.0>
        s += 2;
    158c:	00248513          	add	a0,s1,2
    1590:	b781                	j	14d0 <printf+0x1ae>
                a = "(null)";
    1592:	00001417          	auipc	s0,0x1
    1596:	9de40413          	add	s0,s0,-1570 # 1f70 <__clone+0xba>
    159a:	b74d                	j	153c <printf+0x21a>

000000000000159c <panic>:
#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>

void panic(char *m)
{
    159c:	1141                	add	sp,sp,-16
    159e:	e406                	sd	ra,8(sp)
    puts(m);
    15a0:	d61ff0ef          	jal	1300 <puts>
    exit(-100);
}
    15a4:	60a2                	ld	ra,8(sp)
    exit(-100);
    15a6:	f9c00513          	li	a0,-100
}
    15aa:	0141                	add	sp,sp,16
    exit(-100);
    15ac:	a719                	j	1cb2 <exit>

00000000000015ae <isspace>:
#define HIGHS (ONES * (UCHAR_MAX / 2 + 1))
#define HASZERO(x) (((x)-ONES) & ~(x)&HIGHS)

int isspace(int c)
{
    return c == ' ' || (unsigned)c - '\t' < 5;
    15ae:	02000793          	li	a5,32
    15b2:	00f50663          	beq	a0,a5,15be <isspace+0x10>
    15b6:	355d                	addw	a0,a0,-9
    15b8:	00553513          	sltiu	a0,a0,5
    15bc:	8082                	ret
    15be:	4505                	li	a0,1
}
    15c0:	8082                	ret

00000000000015c2 <isdigit>:

int isdigit(int c)
{
    return (unsigned)c - '0' < 10;
    15c2:	fd05051b          	addw	a0,a0,-48
}
    15c6:	00a53513          	sltiu	a0,a0,10
    15ca:	8082                	ret

00000000000015cc <atoi>:
    return c == ' ' || (unsigned)c - '\t' < 5;
    15cc:	02000693          	li	a3,32
    15d0:	4591                	li	a1,4

int atoi(const char *s)
{
    int n = 0, neg = 0;
    while (isspace(*s))
    15d2:	00054783          	lbu	a5,0(a0)
    return c == ' ' || (unsigned)c - '\t' < 5;
    15d6:	ff77871b          	addw	a4,a5,-9
    15da:	04d78c63          	beq	a5,a3,1632 <atoi+0x66>
    15de:	0007861b          	sext.w	a2,a5
    15e2:	04e5f863          	bgeu	a1,a4,1632 <atoi+0x66>
        s++;
    switch (*s)
    15e6:	02b00713          	li	a4,43
    15ea:	04e78963          	beq	a5,a4,163c <atoi+0x70>
    15ee:	02d00713          	li	a4,45
    15f2:	06e78263          	beq	a5,a4,1656 <atoi+0x8a>
        neg = 1;
    case '+':
        s++;
    }
    /* Compute n as a negative number to avoid overflow on INT_MIN */
    while (isdigit(*s))
    15f6:	fd06069b          	addw	a3,a2,-48
    15fa:	47a5                	li	a5,9
    15fc:	872a                	mv	a4,a0
    int n = 0, neg = 0;
    15fe:	4301                	li	t1,0
    while (isdigit(*s))
    1600:	04d7e963          	bltu	a5,a3,1652 <atoi+0x86>
    int n = 0, neg = 0;
    1604:	4501                	li	a0,0
    while (isdigit(*s))
    1606:	48a5                	li	a7,9
    1608:	00174683          	lbu	a3,1(a4)
        n = 10 * n - (*s++ - '0');
    160c:	0025179b          	sllw	a5,a0,0x2
    1610:	9fa9                	addw	a5,a5,a0
    1612:	fd06059b          	addw	a1,a2,-48
    1616:	0017979b          	sllw	a5,a5,0x1
    while (isdigit(*s))
    161a:	fd06881b          	addw	a6,a3,-48
        n = 10 * n - (*s++ - '0');
    161e:	0705                	add	a4,a4,1
    1620:	40b7853b          	subw	a0,a5,a1
    while (isdigit(*s))
    1624:	0006861b          	sext.w	a2,a3
    1628:	ff08f0e3          	bgeu	a7,a6,1608 <atoi+0x3c>
    return neg ? n : -n;
    162c:	00030563          	beqz	t1,1636 <atoi+0x6a>
}
    1630:	8082                	ret
        s++;
    1632:	0505                	add	a0,a0,1
    1634:	bf79                	j	15d2 <atoi+0x6>
    return neg ? n : -n;
    1636:	40f5853b          	subw	a0,a1,a5
    163a:	8082                	ret
    while (isdigit(*s))
    163c:	00154603          	lbu	a2,1(a0)
    1640:	47a5                	li	a5,9
        s++;
    1642:	00150713          	add	a4,a0,1
    while (isdigit(*s))
    1646:	fd06069b          	addw	a3,a2,-48
    int n = 0, neg = 0;
    164a:	4301                	li	t1,0
    while (isdigit(*s))
    164c:	2601                	sext.w	a2,a2
    164e:	fad7fbe3          	bgeu	a5,a3,1604 <atoi+0x38>
    1652:	4501                	li	a0,0
}
    1654:	8082                	ret
    while (isdigit(*s))
    1656:	00154603          	lbu	a2,1(a0)
    165a:	47a5                	li	a5,9
        s++;
    165c:	00150713          	add	a4,a0,1
    while (isdigit(*s))
    1660:	fd06069b          	addw	a3,a2,-48
    1664:	2601                	sext.w	a2,a2
    1666:	fed7e6e3          	bltu	a5,a3,1652 <atoi+0x86>
        neg = 1;
    166a:	4305                	li	t1,1
    166c:	bf61                	j	1604 <atoi+0x38>

000000000000166e <memset>:

void *memset(void *dest, int c, size_t n)
{
    char *p = dest;
    for (int i = 0; i < n; ++i, *(p++) = c)
    166e:	18060163          	beqz	a2,17f0 <memset+0x182>
    1672:	40a006b3          	neg	a3,a0
    1676:	0076f793          	and	a5,a3,7
    167a:	00778813          	add	a6,a5,7
    167e:	48ad                	li	a7,11
    1680:	0ff5f713          	zext.b	a4,a1
    1684:	fff60593          	add	a1,a2,-1
    1688:	17186563          	bltu	a6,a7,17f2 <memset+0x184>
    168c:	1705ed63          	bltu	a1,a6,1806 <memset+0x198>
    1690:	16078363          	beqz	a5,17f6 <memset+0x188>
    1694:	00e50023          	sb	a4,0(a0)
    1698:	0066f593          	and	a1,a3,6
    169c:	16058063          	beqz	a1,17fc <memset+0x18e>
    16a0:	00e500a3          	sb	a4,1(a0)
    16a4:	4589                	li	a1,2
    16a6:	16f5f363          	bgeu	a1,a5,180c <memset+0x19e>
    16aa:	00e50123          	sb	a4,2(a0)
    16ae:	8a91                	and	a3,a3,4
    16b0:	00350593          	add	a1,a0,3
    16b4:	4e0d                	li	t3,3
    16b6:	ce9d                	beqz	a3,16f4 <memset+0x86>
    16b8:	00e501a3          	sb	a4,3(a0)
    16bc:	4691                	li	a3,4
    16be:	00450593          	add	a1,a0,4
    16c2:	4e11                	li	t3,4
    16c4:	02f6f863          	bgeu	a3,a5,16f4 <memset+0x86>
    16c8:	00e50223          	sb	a4,4(a0)
    16cc:	4695                	li	a3,5
    16ce:	00550593          	add	a1,a0,5
    16d2:	4e15                	li	t3,5
    16d4:	02d78063          	beq	a5,a3,16f4 <memset+0x86>
    16d8:	fff50693          	add	a3,a0,-1
    16dc:	00e502a3          	sb	a4,5(a0)
    16e0:	8a9d                	and	a3,a3,7
    16e2:	00650593          	add	a1,a0,6
    16e6:	4e19                	li	t3,6
    16e8:	e691                	bnez	a3,16f4 <memset+0x86>
    16ea:	00750593          	add	a1,a0,7
    16ee:	00e50323          	sb	a4,6(a0)
    16f2:	4e1d                	li	t3,7
    16f4:	00871693          	sll	a3,a4,0x8
    16f8:	01071813          	sll	a6,a4,0x10
    16fc:	8ed9                	or	a3,a3,a4
    16fe:	01871893          	sll	a7,a4,0x18
    1702:	0106e6b3          	or	a3,a3,a6
    1706:	0116e6b3          	or	a3,a3,a7
    170a:	02071813          	sll	a6,a4,0x20
    170e:	02871313          	sll	t1,a4,0x28
    1712:	0106e6b3          	or	a3,a3,a6
    1716:	40f608b3          	sub	a7,a2,a5
    171a:	03071813          	sll	a6,a4,0x30
    171e:	0066e6b3          	or	a3,a3,t1
    1722:	0106e6b3          	or	a3,a3,a6
    1726:	03871313          	sll	t1,a4,0x38
    172a:	97aa                	add	a5,a5,a0
    172c:	ff88f813          	and	a6,a7,-8
    1730:	0066e6b3          	or	a3,a3,t1
    1734:	983e                	add	a6,a6,a5
    1736:	e394                	sd	a3,0(a5)
    1738:	07a1                	add	a5,a5,8
    173a:	ff079ee3          	bne	a5,a6,1736 <memset+0xc8>
    173e:	ff88f793          	and	a5,a7,-8
    1742:	0078f893          	and	a7,a7,7
    1746:	00f586b3          	add	a3,a1,a5
    174a:	01c787bb          	addw	a5,a5,t3
    174e:	0a088b63          	beqz	a7,1804 <memset+0x196>
    1752:	00e68023          	sb	a4,0(a3)
    1756:	0017859b          	addw	a1,a5,1
    175a:	08c5fb63          	bgeu	a1,a2,17f0 <memset+0x182>
    175e:	00e680a3          	sb	a4,1(a3)
    1762:	0027859b          	addw	a1,a5,2
    1766:	08c5f563          	bgeu	a1,a2,17f0 <memset+0x182>
    176a:	00e68123          	sb	a4,2(a3)
    176e:	0037859b          	addw	a1,a5,3
    1772:	06c5ff63          	bgeu	a1,a2,17f0 <memset+0x182>
    1776:	00e681a3          	sb	a4,3(a3)
    177a:	0047859b          	addw	a1,a5,4
    177e:	06c5f963          	bgeu	a1,a2,17f0 <memset+0x182>
    1782:	00e68223          	sb	a4,4(a3)
    1786:	0057859b          	addw	a1,a5,5
    178a:	06c5f363          	bgeu	a1,a2,17f0 <memset+0x182>
    178e:	00e682a3          	sb	a4,5(a3)
    1792:	0067859b          	addw	a1,a5,6
    1796:	04c5fd63          	bgeu	a1,a2,17f0 <memset+0x182>
    179a:	00e68323          	sb	a4,6(a3)
    179e:	0077859b          	addw	a1,a5,7
    17a2:	04c5f763          	bgeu	a1,a2,17f0 <memset+0x182>
    17a6:	00e683a3          	sb	a4,7(a3)
    17aa:	0087859b          	addw	a1,a5,8
    17ae:	04c5f163          	bgeu	a1,a2,17f0 <memset+0x182>
    17b2:	00e68423          	sb	a4,8(a3)
    17b6:	0097859b          	addw	a1,a5,9
    17ba:	02c5fb63          	bgeu	a1,a2,17f0 <memset+0x182>
    17be:	00e684a3          	sb	a4,9(a3)
    17c2:	00a7859b          	addw	a1,a5,10
    17c6:	02c5f563          	bgeu	a1,a2,17f0 <memset+0x182>
    17ca:	00e68523          	sb	a4,10(a3)
    17ce:	00b7859b          	addw	a1,a5,11
    17d2:	00c5ff63          	bgeu	a1,a2,17f0 <memset+0x182>
    17d6:	00e685a3          	sb	a4,11(a3)
    17da:	00c7859b          	addw	a1,a5,12
    17de:	00c5f963          	bgeu	a1,a2,17f0 <memset+0x182>
    17e2:	00e68623          	sb	a4,12(a3)
    17e6:	27b5                	addw	a5,a5,13
    17e8:	00c7f463          	bgeu	a5,a2,17f0 <memset+0x182>
    17ec:	00e686a3          	sb	a4,13(a3)
        ;
    return dest;
}
    17f0:	8082                	ret
    17f2:	482d                	li	a6,11
    17f4:	bd61                	j	168c <memset+0x1e>
    char *p = dest;
    17f6:	85aa                	mv	a1,a0
    for (int i = 0; i < n; ++i, *(p++) = c)
    17f8:	4e01                	li	t3,0
    17fa:	bded                	j	16f4 <memset+0x86>
    17fc:	00150593          	add	a1,a0,1
    1800:	4e05                	li	t3,1
    1802:	bdcd                	j	16f4 <memset+0x86>
    1804:	8082                	ret
    char *p = dest;
    1806:	86aa                	mv	a3,a0
    for (int i = 0; i < n; ++i, *(p++) = c)
    1808:	4781                	li	a5,0
    180a:	b7a1                	j	1752 <memset+0xe4>
    180c:	00250593          	add	a1,a0,2
    1810:	4e09                	li	t3,2
    1812:	b5cd                	j	16f4 <memset+0x86>

0000000000001814 <strcmp>:

int strcmp(const char *l, const char *r)
{
    for (; *l == *r && *l; l++, r++)
    1814:	00054783          	lbu	a5,0(a0)
    1818:	0005c703          	lbu	a4,0(a1)
    181c:	00e79863          	bne	a5,a4,182c <strcmp+0x18>
    1820:	0505                	add	a0,a0,1
    1822:	0585                	add	a1,a1,1
    1824:	fbe5                	bnez	a5,1814 <strcmp>
    1826:	4501                	li	a0,0
        ;
    return *(unsigned char *)l - *(unsigned char *)r;
}
    1828:	9d19                	subw	a0,a0,a4
    182a:	8082                	ret
    return *(unsigned char *)l - *(unsigned char *)r;
    182c:	0007851b          	sext.w	a0,a5
    1830:	bfe5                	j	1828 <strcmp+0x14>

0000000000001832 <strncmp>:

int strncmp(const char *_l, const char *_r, size_t n)
{
    const unsigned char *l = (void *)_l, *r = (void *)_r;
    if (!n--)
    1832:	ca15                	beqz	a2,1866 <strncmp+0x34>
        return 0;
    for (; *l && *r && n && *l == *r; l++, r++, n--)
    1834:	00054783          	lbu	a5,0(a0)
    if (!n--)
    1838:	167d                	add	a2,a2,-1
    183a:	00c506b3          	add	a3,a0,a2
    for (; *l && *r && n && *l == *r; l++, r++, n--)
    183e:	eb99                	bnez	a5,1854 <strncmp+0x22>
    1840:	a815                	j	1874 <strncmp+0x42>
    1842:	00a68e63          	beq	a3,a0,185e <strncmp+0x2c>
    1846:	0505                	add	a0,a0,1
    1848:	00f71b63          	bne	a4,a5,185e <strncmp+0x2c>
    184c:	00054783          	lbu	a5,0(a0)
    1850:	cf89                	beqz	a5,186a <strncmp+0x38>
    1852:	85b2                	mv	a1,a2
    1854:	0005c703          	lbu	a4,0(a1)
    1858:	00158613          	add	a2,a1,1
    185c:	f37d                	bnez	a4,1842 <strncmp+0x10>
        ;
    return *l - *r;
    185e:	0007851b          	sext.w	a0,a5
    1862:	9d19                	subw	a0,a0,a4
    1864:	8082                	ret
        return 0;
    1866:	4501                	li	a0,0
}
    1868:	8082                	ret
    return *l - *r;
    186a:	0015c703          	lbu	a4,1(a1)
    186e:	4501                	li	a0,0
    1870:	9d19                	subw	a0,a0,a4
    1872:	8082                	ret
    1874:	0005c703          	lbu	a4,0(a1)
    1878:	4501                	li	a0,0
    187a:	b7e5                	j	1862 <strncmp+0x30>

000000000000187c <strlen>:
size_t strlen(const char *s)
{
    const char *a = s;
    typedef size_t __attribute__((__may_alias__)) word;
    const word *w;
    for (; (uintptr_t)s % SS; s++)
    187c:	00757793          	and	a5,a0,7
    1880:	cf89                	beqz	a5,189a <strlen+0x1e>
    1882:	87aa                	mv	a5,a0
    1884:	a029                	j	188e <strlen+0x12>
    1886:	0785                	add	a5,a5,1
    1888:	0077f713          	and	a4,a5,7
    188c:	cb01                	beqz	a4,189c <strlen+0x20>
        if (!*s)
    188e:	0007c703          	lbu	a4,0(a5)
    1892:	fb75                	bnez	a4,1886 <strlen+0xa>
    for (w = (const void *)s; !HASZERO(*w); w++)
        ;
    s = (const void *)w;
    for (; *s; s++)
        ;
    return s - a;
    1894:	40a78533          	sub	a0,a5,a0
}
    1898:	8082                	ret
    for (; (uintptr_t)s % SS; s++)
    189a:	87aa                	mv	a5,a0
    for (w = (const void *)s; !HASZERO(*w); w++)
    189c:	6394                	ld	a3,0(a5)
    189e:	00000597          	auipc	a1,0x0
    18a2:	6da5b583          	ld	a1,1754(a1) # 1f78 <__clone+0xc2>
    18a6:	00000617          	auipc	a2,0x0
    18aa:	6da63603          	ld	a2,1754(a2) # 1f80 <__clone+0xca>
    18ae:	a019                	j	18b4 <strlen+0x38>
    18b0:	6794                	ld	a3,8(a5)
    18b2:	07a1                	add	a5,a5,8
    18b4:	00b68733          	add	a4,a3,a1
    18b8:	fff6c693          	not	a3,a3
    18bc:	8f75                	and	a4,a4,a3
    18be:	8f71                	and	a4,a4,a2
    18c0:	db65                	beqz	a4,18b0 <strlen+0x34>
    for (; *s; s++)
    18c2:	0007c703          	lbu	a4,0(a5)
    18c6:	d779                	beqz	a4,1894 <strlen+0x18>
    18c8:	0017c703          	lbu	a4,1(a5)
    18cc:	0785                	add	a5,a5,1
    18ce:	d379                	beqz	a4,1894 <strlen+0x18>
    18d0:	0017c703          	lbu	a4,1(a5)
    18d4:	0785                	add	a5,a5,1
    18d6:	fb6d                	bnez	a4,18c8 <strlen+0x4c>
    18d8:	bf75                	j	1894 <strlen+0x18>

00000000000018da <memchr>:

void *memchr(const void *src, int c, size_t n)
{
    const unsigned char *s = src;
    c = (unsigned char)c;
    for (; ((uintptr_t)s & ALIGN) && n && *s != c; s++, n--)
    18da:	00757713          	and	a4,a0,7
{
    18de:	87aa                	mv	a5,a0
    18e0:	0ff5f593          	zext.b	a1,a1
    for (; ((uintptr_t)s & ALIGN) && n && *s != c; s++, n--)
    18e4:	cb19                	beqz	a4,18fa <memchr+0x20>
    18e6:	ce25                	beqz	a2,195e <memchr+0x84>
    18e8:	0007c703          	lbu	a4,0(a5)
    18ec:	00b70763          	beq	a4,a1,18fa <memchr+0x20>
    18f0:	0785                	add	a5,a5,1
    18f2:	0077f713          	and	a4,a5,7
    18f6:	167d                	add	a2,a2,-1
    18f8:	f77d                	bnez	a4,18e6 <memchr+0xc>
            ;
        s = (const void *)w;
    }
    for (; n && *s != c; s++, n--)
        ;
    return n ? (void *)s : 0;
    18fa:	4501                	li	a0,0
    if (n && *s != c)
    18fc:	c235                	beqz	a2,1960 <memchr+0x86>
    18fe:	0007c703          	lbu	a4,0(a5)
    1902:	06b70063          	beq	a4,a1,1962 <memchr+0x88>
        size_t k = ONES * c;
    1906:	00000517          	auipc	a0,0x0
    190a:	68253503          	ld	a0,1666(a0) # 1f88 <__clone+0xd2>
        for (w = (const void *)s; n >= SS && !HASZERO(*w ^ k); w++, n -= SS)
    190e:	471d                	li	a4,7
        size_t k = ONES * c;
    1910:	02a58533          	mul	a0,a1,a0
        for (w = (const void *)s; n >= SS && !HASZERO(*w ^ k); w++, n -= SS)
    1914:	04c77763          	bgeu	a4,a2,1962 <memchr+0x88>
    1918:	00000897          	auipc	a7,0x0
    191c:	6608b883          	ld	a7,1632(a7) # 1f78 <__clone+0xc2>
    1920:	00000817          	auipc	a6,0x0
    1924:	66083803          	ld	a6,1632(a6) # 1f80 <__clone+0xca>
    1928:	431d                	li	t1,7
    192a:	a029                	j	1934 <memchr+0x5a>
    192c:	1661                	add	a2,a2,-8
    192e:	07a1                	add	a5,a5,8
    1930:	00c37c63          	bgeu	t1,a2,1948 <memchr+0x6e>
    1934:	6398                	ld	a4,0(a5)
    1936:	8f29                	xor	a4,a4,a0
    1938:	011706b3          	add	a3,a4,a7
    193c:	fff74713          	not	a4,a4
    1940:	8f75                	and	a4,a4,a3
    1942:	01077733          	and	a4,a4,a6
    1946:	d37d                	beqz	a4,192c <memchr+0x52>
    1948:	853e                	mv	a0,a5
    for (; n && *s != c; s++, n--)
    194a:	e601                	bnez	a2,1952 <memchr+0x78>
    194c:	a809                	j	195e <memchr+0x84>
    194e:	0505                	add	a0,a0,1
    1950:	c619                	beqz	a2,195e <memchr+0x84>
    1952:	00054783          	lbu	a5,0(a0)
    1956:	167d                	add	a2,a2,-1
    1958:	feb79be3          	bne	a5,a1,194e <memchr+0x74>
    195c:	8082                	ret
    return n ? (void *)s : 0;
    195e:	4501                	li	a0,0
}
    1960:	8082                	ret
    if (n && *s != c)
    1962:	853e                	mv	a0,a5
    1964:	b7fd                	j	1952 <memchr+0x78>

0000000000001966 <strnlen>:

size_t strnlen(const char *s, size_t n)
{
    1966:	1101                	add	sp,sp,-32
    1968:	e822                	sd	s0,16(sp)
    const char *p = memchr(s, 0, n);
    196a:	862e                	mv	a2,a1
{
    196c:	842e                	mv	s0,a1
    const char *p = memchr(s, 0, n);
    196e:	4581                	li	a1,0
{
    1970:	e426                	sd	s1,8(sp)
    1972:	ec06                	sd	ra,24(sp)
    1974:	84aa                	mv	s1,a0
    const char *p = memchr(s, 0, n);
    1976:	f65ff0ef          	jal	18da <memchr>
    return p ? p - s : n;
    197a:	c519                	beqz	a0,1988 <strnlen+0x22>
}
    197c:	60e2                	ld	ra,24(sp)
    197e:	6442                	ld	s0,16(sp)
    return p ? p - s : n;
    1980:	8d05                	sub	a0,a0,s1
}
    1982:	64a2                	ld	s1,8(sp)
    1984:	6105                	add	sp,sp,32
    1986:	8082                	ret
    1988:	60e2                	ld	ra,24(sp)
    return p ? p - s : n;
    198a:	8522                	mv	a0,s0
}
    198c:	6442                	ld	s0,16(sp)
    198e:	64a2                	ld	s1,8(sp)
    1990:	6105                	add	sp,sp,32
    1992:	8082                	ret

0000000000001994 <strcpy>:
char *strcpy(char *restrict d, const char *s)
{
    typedef size_t __attribute__((__may_alias__)) word;
    word *wd;
    const word *ws;
    if ((uintptr_t)s % SS == (uintptr_t)d % SS)
    1994:	00a5c7b3          	xor	a5,a1,a0
    1998:	8b9d                	and	a5,a5,7
    199a:	eb95                	bnez	a5,19ce <strcpy+0x3a>
    {
        for (; (uintptr_t)s % SS; s++, d++)
    199c:	0075f793          	and	a5,a1,7
    19a0:	e7b1                	bnez	a5,19ec <strcpy+0x58>
            if (!(*d = *s))
                return d;
        wd = (void *)d;
        ws = (const void *)s;
        for (; !HASZERO(*ws); *wd++ = *ws++)
    19a2:	6198                	ld	a4,0(a1)
    19a4:	00000617          	auipc	a2,0x0
    19a8:	5d463603          	ld	a2,1492(a2) # 1f78 <__clone+0xc2>
    19ac:	00000817          	auipc	a6,0x0
    19b0:	5d483803          	ld	a6,1492(a6) # 1f80 <__clone+0xca>
    19b4:	a029                	j	19be <strcpy+0x2a>
    19b6:	05a1                	add	a1,a1,8
    19b8:	e118                	sd	a4,0(a0)
    19ba:	6198                	ld	a4,0(a1)
    19bc:	0521                	add	a0,a0,8
    19be:	00c707b3          	add	a5,a4,a2
    19c2:	fff74693          	not	a3,a4
    19c6:	8ff5                	and	a5,a5,a3
    19c8:	0107f7b3          	and	a5,a5,a6
    19cc:	d7ed                	beqz	a5,19b6 <strcpy+0x22>
            ;
        d = (void *)wd;
        s = (const void *)ws;
    }
    for (; (*d = *s); s++, d++)
    19ce:	0005c783          	lbu	a5,0(a1)
    19d2:	00f50023          	sb	a5,0(a0)
    19d6:	c785                	beqz	a5,19fe <strcpy+0x6a>
    19d8:	0015c783          	lbu	a5,1(a1)
    19dc:	0505                	add	a0,a0,1
    19de:	0585                	add	a1,a1,1
    19e0:	00f50023          	sb	a5,0(a0)
    19e4:	fbf5                	bnez	a5,19d8 <strcpy+0x44>
        ;
    return d;
}
    19e6:	8082                	ret
        for (; (uintptr_t)s % SS; s++, d++)
    19e8:	0505                	add	a0,a0,1
    19ea:	df45                	beqz	a4,19a2 <strcpy+0xe>
            if (!(*d = *s))
    19ec:	0005c783          	lbu	a5,0(a1)
        for (; (uintptr_t)s % SS; s++, d++)
    19f0:	0585                	add	a1,a1,1
    19f2:	0075f713          	and	a4,a1,7
            if (!(*d = *s))
    19f6:	00f50023          	sb	a5,0(a0)
    19fa:	f7fd                	bnez	a5,19e8 <strcpy+0x54>
}
    19fc:	8082                	ret
    19fe:	8082                	ret

0000000000001a00 <strncpy>:
char *strncpy(char *restrict d, const char *s, size_t n)
{
    typedef size_t __attribute__((__may_alias__)) word;
    word *wd;
    const word *ws;
    if (((uintptr_t)s & ALIGN) == ((uintptr_t)d & ALIGN))
    1a00:	00a5c7b3          	xor	a5,a1,a0
    1a04:	8b9d                	and	a5,a5,7
    1a06:	e3b5                	bnez	a5,1a6a <strncpy+0x6a>
    {
        for (; ((uintptr_t)s & ALIGN) && n && (*d = *s); n--, s++, d++)
    1a08:	0075f793          	and	a5,a1,7
    1a0c:	cf99                	beqz	a5,1a2a <strncpy+0x2a>
    1a0e:	ea09                	bnez	a2,1a20 <strncpy+0x20>
    1a10:	a421                	j	1c18 <strncpy+0x218>
    1a12:	0585                	add	a1,a1,1
    1a14:	0075f793          	and	a5,a1,7
    1a18:	167d                	add	a2,a2,-1
    1a1a:	0505                	add	a0,a0,1
    1a1c:	c799                	beqz	a5,1a2a <strncpy+0x2a>
    1a1e:	c225                	beqz	a2,1a7e <strncpy+0x7e>
    1a20:	0005c783          	lbu	a5,0(a1)
    1a24:	00f50023          	sb	a5,0(a0)
    1a28:	f7ed                	bnez	a5,1a12 <strncpy+0x12>
            ;
        if (!n || !*s)
    1a2a:	ca31                	beqz	a2,1a7e <strncpy+0x7e>
    1a2c:	0005c783          	lbu	a5,0(a1)
    1a30:	cba1                	beqz	a5,1a80 <strncpy+0x80>
            goto tail;
        wd = (void *)d;
        ws = (const void *)s;
        for (; n >= sizeof(size_t) && !HASZERO(*ws); n -= sizeof(size_t), ws++, wd++)
    1a32:	479d                	li	a5,7
    1a34:	02c7fc63          	bgeu	a5,a2,1a6c <strncpy+0x6c>
    1a38:	00000897          	auipc	a7,0x0
    1a3c:	5408b883          	ld	a7,1344(a7) # 1f78 <__clone+0xc2>
    1a40:	00000817          	auipc	a6,0x0
    1a44:	54083803          	ld	a6,1344(a6) # 1f80 <__clone+0xca>
    1a48:	431d                	li	t1,7
    1a4a:	a039                	j	1a58 <strncpy+0x58>
            *wd = *ws;
    1a4c:	e118                	sd	a4,0(a0)
        for (; n >= sizeof(size_t) && !HASZERO(*ws); n -= sizeof(size_t), ws++, wd++)
    1a4e:	1661                	add	a2,a2,-8
    1a50:	05a1                	add	a1,a1,8
    1a52:	0521                	add	a0,a0,8
    1a54:	00c37b63          	bgeu	t1,a2,1a6a <strncpy+0x6a>
    1a58:	6198                	ld	a4,0(a1)
    1a5a:	011707b3          	add	a5,a4,a7
    1a5e:	fff74693          	not	a3,a4
    1a62:	8ff5                	and	a5,a5,a3
    1a64:	0107f7b3          	and	a5,a5,a6
    1a68:	d3f5                	beqz	a5,1a4c <strncpy+0x4c>
        d = (void *)wd;
        s = (const void *)ws;
    }
    for (; n && (*d = *s); n--, s++, d++)
    1a6a:	ca11                	beqz	a2,1a7e <strncpy+0x7e>
    1a6c:	0005c783          	lbu	a5,0(a1)
    1a70:	0585                	add	a1,a1,1
    1a72:	00f50023          	sb	a5,0(a0)
    1a76:	c789                	beqz	a5,1a80 <strncpy+0x80>
    1a78:	167d                	add	a2,a2,-1
    1a7a:	0505                	add	a0,a0,1
    1a7c:	fa65                	bnez	a2,1a6c <strncpy+0x6c>
        ;
tail:
    memset(d, 0, n);
    return d;
}
    1a7e:	8082                	ret
    1a80:	4805                	li	a6,1
    1a82:	14061b63          	bnez	a2,1bd8 <strncpy+0x1d8>
    1a86:	40a00733          	neg	a4,a0
    1a8a:	00777793          	and	a5,a4,7
    1a8e:	4581                	li	a1,0
    1a90:	12061c63          	bnez	a2,1bc8 <strncpy+0x1c8>
    1a94:	00778693          	add	a3,a5,7
    1a98:	48ad                	li	a7,11
    1a9a:	1316e563          	bltu	a3,a7,1bc4 <strncpy+0x1c4>
    1a9e:	16d5e263          	bltu	a1,a3,1c02 <strncpy+0x202>
    1aa2:	14078c63          	beqz	a5,1bfa <strncpy+0x1fa>
    for (int i = 0; i < n; ++i, *(p++) = c)
    1aa6:	00050023          	sb	zero,0(a0)
    1aaa:	00677693          	and	a3,a4,6
    1aae:	14068263          	beqz	a3,1bf2 <strncpy+0x1f2>
    1ab2:	000500a3          	sb	zero,1(a0)
    1ab6:	4689                	li	a3,2
    1ab8:	14f6f863          	bgeu	a3,a5,1c08 <strncpy+0x208>
    1abc:	00050123          	sb	zero,2(a0)
    1ac0:	8b11                	and	a4,a4,4
    1ac2:	12070463          	beqz	a4,1bea <strncpy+0x1ea>
    1ac6:	000501a3          	sb	zero,3(a0)
    1aca:	4711                	li	a4,4
    1acc:	00450693          	add	a3,a0,4
    1ad0:	02f77563          	bgeu	a4,a5,1afa <strncpy+0xfa>
    1ad4:	00050223          	sb	zero,4(a0)
    1ad8:	4715                	li	a4,5
    1ada:	00550693          	add	a3,a0,5
    1ade:	00e78e63          	beq	a5,a4,1afa <strncpy+0xfa>
    1ae2:	fff50713          	add	a4,a0,-1
    1ae6:	000502a3          	sb	zero,5(a0)
    1aea:	8b1d                	and	a4,a4,7
    1aec:	12071263          	bnez	a4,1c10 <strncpy+0x210>
    1af0:	00750693          	add	a3,a0,7
    1af4:	00050323          	sb	zero,6(a0)
    1af8:	471d                	li	a4,7
    1afa:	40f80833          	sub	a6,a6,a5
    1afe:	ff887593          	and	a1,a6,-8
    1b02:	97aa                	add	a5,a5,a0
    1b04:	95be                	add	a1,a1,a5
    1b06:	0007b023          	sd	zero,0(a5)
    1b0a:	07a1                	add	a5,a5,8
    1b0c:	feb79de3          	bne	a5,a1,1b06 <strncpy+0x106>
    1b10:	ff887593          	and	a1,a6,-8
    1b14:	00787813          	and	a6,a6,7
    1b18:	00e587bb          	addw	a5,a1,a4
    1b1c:	00b68733          	add	a4,a3,a1
    1b20:	0e080063          	beqz	a6,1c00 <strncpy+0x200>
    1b24:	00070023          	sb	zero,0(a4)
    1b28:	0017869b          	addw	a3,a5,1
    1b2c:	f4c6f9e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b30:	000700a3          	sb	zero,1(a4)
    1b34:	0027869b          	addw	a3,a5,2
    1b38:	f4c6f3e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b3c:	00070123          	sb	zero,2(a4)
    1b40:	0037869b          	addw	a3,a5,3
    1b44:	f2c6fde3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b48:	000701a3          	sb	zero,3(a4)
    1b4c:	0047869b          	addw	a3,a5,4
    1b50:	f2c6f7e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b54:	00070223          	sb	zero,4(a4)
    1b58:	0057869b          	addw	a3,a5,5
    1b5c:	f2c6f1e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b60:	000702a3          	sb	zero,5(a4)
    1b64:	0067869b          	addw	a3,a5,6
    1b68:	f0c6fbe3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b6c:	00070323          	sb	zero,6(a4)
    1b70:	0077869b          	addw	a3,a5,7
    1b74:	f0c6f5e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b78:	000703a3          	sb	zero,7(a4)
    1b7c:	0087869b          	addw	a3,a5,8
    1b80:	eec6ffe3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b84:	00070423          	sb	zero,8(a4)
    1b88:	0097869b          	addw	a3,a5,9
    1b8c:	eec6f9e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b90:	000704a3          	sb	zero,9(a4)
    1b94:	00a7869b          	addw	a3,a5,10
    1b98:	eec6f3e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1b9c:	00070523          	sb	zero,10(a4)
    1ba0:	00b7869b          	addw	a3,a5,11
    1ba4:	ecc6fde3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1ba8:	000705a3          	sb	zero,11(a4)
    1bac:	00c7869b          	addw	a3,a5,12
    1bb0:	ecc6f7e3          	bgeu	a3,a2,1a7e <strncpy+0x7e>
    1bb4:	00070623          	sb	zero,12(a4)
    1bb8:	27b5                	addw	a5,a5,13
    1bba:	ecc7f2e3          	bgeu	a5,a2,1a7e <strncpy+0x7e>
    1bbe:	000706a3          	sb	zero,13(a4)
}
    1bc2:	8082                	ret
    1bc4:	46ad                	li	a3,11
    1bc6:	bde1                	j	1a9e <strncpy+0x9e>
    1bc8:	00778693          	add	a3,a5,7
    1bcc:	48ad                	li	a7,11
    1bce:	fff60593          	add	a1,a2,-1
    1bd2:	ed16f6e3          	bgeu	a3,a7,1a9e <strncpy+0x9e>
    1bd6:	b7fd                	j	1bc4 <strncpy+0x1c4>
    1bd8:	40a00733          	neg	a4,a0
    1bdc:	8832                	mv	a6,a2
    1bde:	00777793          	and	a5,a4,7
    1be2:	4581                	li	a1,0
    1be4:	ea0608e3          	beqz	a2,1a94 <strncpy+0x94>
    1be8:	b7c5                	j	1bc8 <strncpy+0x1c8>
    for (int i = 0; i < n; ++i, *(p++) = c)
    1bea:	00350693          	add	a3,a0,3
    1bee:	470d                	li	a4,3
    1bf0:	b729                	j	1afa <strncpy+0xfa>
    1bf2:	00150693          	add	a3,a0,1
    1bf6:	4705                	li	a4,1
    1bf8:	b709                	j	1afa <strncpy+0xfa>
tail:
    1bfa:	86aa                	mv	a3,a0
    for (int i = 0; i < n; ++i, *(p++) = c)
    1bfc:	4701                	li	a4,0
    1bfe:	bdf5                	j	1afa <strncpy+0xfa>
    1c00:	8082                	ret
tail:
    1c02:	872a                	mv	a4,a0
    for (int i = 0; i < n; ++i, *(p++) = c)
    1c04:	4781                	li	a5,0
    1c06:	bf39                	j	1b24 <strncpy+0x124>
    1c08:	00250693          	add	a3,a0,2
    1c0c:	4709                	li	a4,2
    1c0e:	b5f5                	j	1afa <strncpy+0xfa>
    1c10:	00650693          	add	a3,a0,6
    1c14:	4719                	li	a4,6
    1c16:	b5d5                	j	1afa <strncpy+0xfa>
    1c18:	8082                	ret

0000000000001c1a <open>:
#include <unistd.h>

#include "syscall.h"

int open(const char *path, int flags)
{
    1c1a:	87aa                	mv	a5,a0
    1c1c:	862e                	mv	a2,a1
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
}

static inline long __syscall4(long n, long a, long b, long c, long d)
{
    register long a7 __asm__("a7") = n;
    1c1e:	03800893          	li	a7,56
    register long a0 __asm__("a0") = a;
    1c22:	f9c00513          	li	a0,-100
    register long a1 __asm__("a1") = b;
    1c26:	85be                	mv	a1,a5
    register long a2 __asm__("a2") = c;
    register long a3 __asm__("a3") = d;
    1c28:	4689                	li	a3,2
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3))
    1c2a:	00000073          	ecall
    return syscall(SYS_openat, AT_FDCWD, path, flags, O_RDWR);
}
    1c2e:	2501                	sext.w	a0,a0
    1c30:	8082                	ret

0000000000001c32 <openat>:
    register long a7 __asm__("a7") = n;
    1c32:	03800893          	li	a7,56
    register long a3 __asm__("a3") = d;
    1c36:	18000693          	li	a3,384
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3))
    1c3a:	00000073          	ecall

int openat(int dirfd,const char *path, int flags)
{
    return syscall(SYS_openat, dirfd, path, flags, 0600);
}
    1c3e:	2501                	sext.w	a0,a0
    1c40:	8082                	ret

0000000000001c42 <close>:
    register long a7 __asm__("a7") = n;
    1c42:	03900893          	li	a7,57
    __asm_syscall("r"(a7), "0"(a0))
    1c46:	00000073          	ecall

int close(int fd)
{
    return syscall(SYS_close, fd);
}
    1c4a:	2501                	sext.w	a0,a0
    1c4c:	8082                	ret

0000000000001c4e <read>:
    register long a7 __asm__("a7") = n;
    1c4e:	03f00893          	li	a7,63
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1c52:	00000073          	ecall

ssize_t read(int fd, void *buf, size_t len)
{
    return syscall(SYS_read, fd, buf, len);
}
    1c56:	8082                	ret

0000000000001c58 <write>:
    register long a7 __asm__("a7") = n;
    1c58:	04000893          	li	a7,64
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1c5c:	00000073          	ecall

ssize_t write(int fd, const void *buf, size_t len)
{
    return syscall(SYS_write, fd, buf, len);
}
    1c60:	8082                	ret

0000000000001c62 <getpid>:
    register long a7 __asm__("a7") = n;
    1c62:	0ac00893          	li	a7,172
    __asm_syscall("r"(a7))
    1c66:	00000073          	ecall

pid_t getpid(void)
{
    return syscall(SYS_getpid);
}
    1c6a:	2501                	sext.w	a0,a0
    1c6c:	8082                	ret

0000000000001c6e <getppid>:
    register long a7 __asm__("a7") = n;
    1c6e:	0ad00893          	li	a7,173
    __asm_syscall("r"(a7))
    1c72:	00000073          	ecall

pid_t getppid(void)
{
    return syscall(SYS_getppid);
}
    1c76:	2501                	sext.w	a0,a0
    1c78:	8082                	ret

0000000000001c7a <sched_yield>:
    register long a7 __asm__("a7") = n;
    1c7a:	07c00893          	li	a7,124
    __asm_syscall("r"(a7))
    1c7e:	00000073          	ecall

int sched_yield(void)
{
    return syscall(SYS_sched_yield);
}
    1c82:	2501                	sext.w	a0,a0
    1c84:	8082                	ret

0000000000001c86 <fork>:
    register long a7 __asm__("a7") = n;
    1c86:	0dc00893          	li	a7,220
    register long a0 __asm__("a0") = a;
    1c8a:	4545                	li	a0,17
    register long a1 __asm__("a1") = b;
    1c8c:	4581                	li	a1,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1c8e:	00000073          	ecall

pid_t fork(void)
{
    return syscall(SYS_clone, SIGCHLD, 0);
}
    1c92:	2501                	sext.w	a0,a0
    1c94:	8082                	ret

0000000000001c96 <clone>:

pid_t clone(int (*fn)(void *arg), void *arg, void *stack, size_t stack_size, unsigned long flags)
{
    1c96:	85b2                	mv	a1,a2
    1c98:	863a                	mv	a2,a4
    if (stack)
    1c9a:	c191                	beqz	a1,1c9e <clone+0x8>
	stack += stack_size;
    1c9c:	95b6                	add	a1,a1,a3

    return __clone(fn, stack, flags, NULL, NULL, NULL);
    1c9e:	4781                	li	a5,0
    1ca0:	4701                	li	a4,0
    1ca2:	4681                	li	a3,0
    1ca4:	2601                	sext.w	a2,a2
    1ca6:	ac01                	j	1eb6 <__clone>

0000000000001ca8 <sys_clone_raw>:
}

static inline long __syscall5(long n, long a, long b, long c, long d, long e)
{
    register long a7 __asm__("a7") = n;
    1ca8:	0dc00893          	li	a7,220
    register long a0 __asm__("a0") = a;
    register long a1 __asm__("a1") = b;
    register long a2 __asm__("a2") = c;
    register long a3 __asm__("a3") = d;
    register long a4 __asm__("a4") = e;
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4))
    1cac:	00000073          	ecall
}

long sys_clone_raw(unsigned long flags, void *stack, int *ptid, void *tls, int *ctid)
{
    return syscall(SYS_clone, flags, stack, ptid, tls, ctid);
}
    1cb0:	8082                	ret

0000000000001cb2 <exit>:
    register long a7 __asm__("a7") = n;
    1cb2:	05d00893          	li	a7,93
    __asm_syscall("r"(a7), "0"(a0))
    1cb6:	00000073          	ecall
void exit(int code)
{
    syscall(SYS_exit, code);
}
    1cba:	8082                	ret

0000000000001cbc <waitpid>:
    register long a7 __asm__("a7") = n;
    1cbc:	10400893          	li	a7,260
    register long a3 __asm__("a3") = d;
    1cc0:	4681                	li	a3,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3))
    1cc2:	00000073          	ecall

int waitpid(int pid, int *code, int options)
{
    return syscall(SYS_wait4, pid, code, options, 0);
}
    1cc6:	2501                	sext.w	a0,a0
    1cc8:	8082                	ret

0000000000001cca <exec>:
    register long a7 __asm__("a7") = n;
    1cca:	0dd00893          	li	a7,221
    __asm_syscall("r"(a7), "0"(a0))
    1cce:	00000073          	ecall

int exec(char *name)
{
    return syscall(SYS_execve, name);
}
    1cd2:	2501                	sext.w	a0,a0
    1cd4:	8082                	ret

0000000000001cd6 <execve>:
    register long a7 __asm__("a7") = n;
    1cd6:	0dd00893          	li	a7,221
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1cda:	00000073          	ecall

int execve(const char *name, char *const argv[], char *const argp[])
{
    return syscall(SYS_execve, name, argv, argp);
}
    1cde:	2501                	sext.w	a0,a0
    1ce0:	8082                	ret

0000000000001ce2 <times>:
    register long a7 __asm__("a7") = n;
    1ce2:	09900893          	li	a7,153
    __asm_syscall("r"(a7), "0"(a0))
    1ce6:	00000073          	ecall

int times(void *mytimes)
{
	return syscall(SYS_times, mytimes);
}
    1cea:	2501                	sext.w	a0,a0
    1cec:	8082                	ret

0000000000001cee <get_time>:

int64 get_time()
{
    1cee:	1141                	add	sp,sp,-16
    register long a7 __asm__("a7") = n;
    1cf0:	0a900893          	li	a7,169
    register long a0 __asm__("a0") = a;
    1cf4:	850a                	mv	a0,sp
    register long a1 __asm__("a1") = b;
    1cf6:	4581                	li	a1,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1cf8:	00000073          	ecall
    TimeVal time;
    int err = sys_get_time(&time, 0);
    if (err == 0)
    1cfc:	2501                	sext.w	a0,a0
    1cfe:	ed09                	bnez	a0,1d18 <get_time+0x2a>
    {
        return ((time.sec & 0xffff) * 1000 + time.usec / 1000);
    1d00:	67a2                	ld	a5,8(sp)
    1d02:	3e800713          	li	a4,1000
    1d06:	00015503          	lhu	a0,0(sp)
    1d0a:	02e7d7b3          	divu	a5,a5,a4
    1d0e:	02e50533          	mul	a0,a0,a4
    1d12:	953e                	add	a0,a0,a5
    }
    else
    {
        return -1;
    }
}
    1d14:	0141                	add	sp,sp,16
    1d16:	8082                	ret
        return -1;
    1d18:	557d                	li	a0,-1
    1d1a:	bfed                	j	1d14 <get_time+0x26>

0000000000001d1c <sys_get_time>:
    register long a7 __asm__("a7") = n;
    1d1c:	0a900893          	li	a7,169
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1d20:	00000073          	ecall

int sys_get_time(TimeVal *ts, int tz)
{
    return syscall(SYS_gettimeofday, ts, tz);
}
    1d24:	2501                	sext.w	a0,a0
    1d26:	8082                	ret

0000000000001d28 <time>:
    register long a7 __asm__("a7") = n;
    1d28:	42600893          	li	a7,1062
    __asm_syscall("r"(a7), "0"(a0))
    1d2c:	00000073          	ecall

int time(unsigned long *tloc)
{
    return syscall(SYS_time, tloc);
}
    1d30:	2501                	sext.w	a0,a0
    1d32:	8082                	ret

0000000000001d34 <sleep>:

int sleep(unsigned long long time)
{
    1d34:	1141                	add	sp,sp,-16
    TimeVal tv = {.sec = time, .usec = 0};
    1d36:	e02a                	sd	a0,0(sp)
    register long a0 __asm__("a0") = a;
    1d38:	850a                	mv	a0,sp
    1d3a:	e402                	sd	zero,8(sp)
    register long a7 __asm__("a7") = n;
    1d3c:	06500893          	li	a7,101
    register long a1 __asm__("a1") = b;
    1d40:	85aa                	mv	a1,a0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1d42:	00000073          	ecall
    if (syscall(SYS_nanosleep, &tv, &tv)) return tv.sec;
    1d46:	e501                	bnez	a0,1d4e <sleep+0x1a>
    return 0;
    1d48:	4501                	li	a0,0
}
    1d4a:	0141                	add	sp,sp,16
    1d4c:	8082                	ret
    if (syscall(SYS_nanosleep, &tv, &tv)) return tv.sec;
    1d4e:	4502                	lw	a0,0(sp)
}
    1d50:	0141                	add	sp,sp,16
    1d52:	8082                	ret

0000000000001d54 <set_priority>:
    register long a7 __asm__("a7") = n;
    1d54:	08c00893          	li	a7,140
    __asm_syscall("r"(a7), "0"(a0))
    1d58:	00000073          	ecall

int set_priority(int prio)
{
    return syscall(SYS_setpriority, prio);
}
    1d5c:	2501                	sext.w	a0,a0
    1d5e:	8082                	ret

0000000000001d60 <mmap>:
}

static inline long __syscall6(long n, long a, long b, long c, long d, long e, long f)
{
    register long a7 __asm__("a7") = n;
    1d60:	0de00893          	li	a7,222
    register long a1 __asm__("a1") = b;
    register long a2 __asm__("a2") = c;
    register long a3 __asm__("a3") = d;
    register long a4 __asm__("a4") = e;
    register long a5 __asm__("a5") = f;
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4), "r"(a5))
    1d64:	00000073          	ecall

void *mmap(void *start, size_t len, int prot, int flags, int fd, off_t off)
{
    return syscall(SYS_mmap, start, len, prot, flags, fd, off);
}
    1d68:	8082                	ret

0000000000001d6a <mprotect>:
    register long a7 __asm__("a7") = n;
    1d6a:	0e200893          	li	a7,226
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1d6e:	00000073          	ecall

int mprotect(void *addr, size_t len, int prot)
{
    return syscall(SYS_mprotect, addr, len, prot);
}
    1d72:	2501                	sext.w	a0,a0
    1d74:	8082                	ret

0000000000001d76 <munmap>:
    register long a7 __asm__("a7") = n;
    1d76:	0d700893          	li	a7,215
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1d7a:	00000073          	ecall

int munmap(void *start, size_t len)
{
    return syscall(SYS_munmap, start, len);
}
    1d7e:	2501                	sext.w	a0,a0
    1d80:	8082                	ret

0000000000001d82 <wait>:

int wait(int *code)
{
    1d82:	85aa                	mv	a1,a0
    register long a7 __asm__("a7") = n;
    1d84:	10400893          	li	a7,260
    register long a0 __asm__("a0") = a;
    1d88:	557d                	li	a0,-1
    register long a2 __asm__("a2") = c;
    1d8a:	4601                	li	a2,0
    register long a3 __asm__("a3") = d;
    1d8c:	4681                	li	a3,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3))
    1d8e:	00000073          	ecall
    return waitpid((int)-1, code, 0);
}
    1d92:	2501                	sext.w	a0,a0
    1d94:	8082                	ret

0000000000001d96 <spawn>:
    register long a7 __asm__("a7") = n;
    1d96:	19000893          	li	a7,400
    __asm_syscall("r"(a7), "0"(a0))
    1d9a:	00000073          	ecall

int spawn(char *file)
{
    return syscall(SYS_spawn, file);
}
    1d9e:	2501                	sext.w	a0,a0
    1da0:	8082                	ret

0000000000001da2 <mailread>:
    register long a7 __asm__("a7") = n;
    1da2:	19100893          	li	a7,401
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1da6:	00000073          	ecall

int mailread(void *buf, int len)
{
    return syscall(SYS_mailread, buf, len);
}
    1daa:	2501                	sext.w	a0,a0
    1dac:	8082                	ret

0000000000001dae <mailwrite>:
    register long a7 __asm__("a7") = n;
    1dae:	19200893          	li	a7,402
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1db2:	00000073          	ecall

int mailwrite(int pid, void *buf, int len)
{
    return syscall(SYS_mailwrite, pid, buf, len);
}
    1db6:	2501                	sext.w	a0,a0
    1db8:	8082                	ret

0000000000001dba <fstat>:
    register long a7 __asm__("a7") = n;
    1dba:	05000893          	li	a7,80
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1dbe:	00000073          	ecall

int fstat(int fd, struct kstat *st)
{
    return syscall(SYS_fstat, fd, st);
}
    1dc2:	2501                	sext.w	a0,a0
    1dc4:	8082                	ret

0000000000001dc6 <sys_mkdirat>:
    register long a2 __asm__("a2") = c;
    1dc6:	1602                	sll	a2,a2,0x20
    register long a7 __asm__("a7") = n;
    1dc8:	02200893          	li	a7,34
    register long a2 __asm__("a2") = c;
    1dcc:	9201                	srl	a2,a2,0x20
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1dce:	00000073          	ecall

int sys_mkdirat(int dirfd, const char *path, mode_t mode)
{
    return syscall(SYS_mkdirat, dirfd, path, mode);
}
    1dd2:	2501                	sext.w	a0,a0
    1dd4:	8082                	ret

0000000000001dd6 <sys_linkat>:
    register long a4 __asm__("a4") = e;
    1dd6:	1702                	sll	a4,a4,0x20
    register long a7 __asm__("a7") = n;
    1dd8:	02500893          	li	a7,37
    register long a4 __asm__("a4") = e;
    1ddc:	9301                	srl	a4,a4,0x20
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4))
    1dde:	00000073          	ecall

int sys_linkat(int olddirfd, char *oldpath, int newdirfd, char *newpath, unsigned int flags)
{
    return syscall(SYS_linkat, olddirfd, oldpath, newdirfd, newpath, flags);
}
    1de2:	2501                	sext.w	a0,a0
    1de4:	8082                	ret

0000000000001de6 <sys_unlinkat>:
    register long a2 __asm__("a2") = c;
    1de6:	1602                	sll	a2,a2,0x20
    register long a7 __asm__("a7") = n;
    1de8:	02300893          	li	a7,35
    register long a2 __asm__("a2") = c;
    1dec:	9201                	srl	a2,a2,0x20
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1dee:	00000073          	ecall

int sys_unlinkat(int dirfd, char *path, unsigned int flags)
{
    return syscall(SYS_unlinkat, dirfd, path, flags);
}
    1df2:	2501                	sext.w	a0,a0
    1df4:	8082                	ret

0000000000001df6 <link>:

int link(char *old_path, char *new_path)
{
    1df6:	87aa                	mv	a5,a0
    1df8:	86ae                	mv	a3,a1
    register long a7 __asm__("a7") = n;
    1dfa:	02500893          	li	a7,37
    register long a0 __asm__("a0") = a;
    1dfe:	f9c00513          	li	a0,-100
    register long a1 __asm__("a1") = b;
    1e02:	85be                	mv	a1,a5
    register long a2 __asm__("a2") = c;
    1e04:	f9c00613          	li	a2,-100
    register long a4 __asm__("a4") = e;
    1e08:	4701                	li	a4,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4))
    1e0a:	00000073          	ecall
    return sys_linkat(AT_FDCWD, old_path, AT_FDCWD, new_path, 0);
}
    1e0e:	2501                	sext.w	a0,a0
    1e10:	8082                	ret

0000000000001e12 <unlink>:

int unlink(char *path)
{
    1e12:	85aa                	mv	a1,a0
    register long a7 __asm__("a7") = n;
    1e14:	02300893          	li	a7,35
    register long a0 __asm__("a0") = a;
    1e18:	f9c00513          	li	a0,-100
    register long a2 __asm__("a2") = c;
    1e1c:	4601                	li	a2,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1e1e:	00000073          	ecall
    return sys_unlinkat(AT_FDCWD, path, 0);
}
    1e22:	2501                	sext.w	a0,a0
    1e24:	8082                	ret

0000000000001e26 <uname>:
    register long a7 __asm__("a7") = n;
    1e26:	0a000893          	li	a7,160
    __asm_syscall("r"(a7), "0"(a0))
    1e2a:	00000073          	ecall

int uname(void *buf)
{
    return syscall(SYS_uname, buf);
}
    1e2e:	2501                	sext.w	a0,a0
    1e30:	8082                	ret

0000000000001e32 <brk>:
    register long a7 __asm__("a7") = n;
    1e32:	0d600893          	li	a7,214
    __asm_syscall("r"(a7), "0"(a0))
    1e36:	00000073          	ecall

int brk(void *addr)
{
    return syscall(SYS_brk, addr);
}
    1e3a:	2501                	sext.w	a0,a0
    1e3c:	8082                	ret

0000000000001e3e <getcwd>:
    register long a7 __asm__("a7") = n;
    1e3e:	48c5                	li	a7,17
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1e40:	00000073          	ecall

char *getcwd(char *buf, size_t size){
    return syscall(SYS_getcwd, buf, size);
}
    1e44:	8082                	ret

0000000000001e46 <chdir>:
    register long a7 __asm__("a7") = n;
    1e46:	03100893          	li	a7,49
    __asm_syscall("r"(a7), "0"(a0))
    1e4a:	00000073          	ecall

int chdir(const char *path){
    return syscall(SYS_chdir, path);
}
    1e4e:	2501                	sext.w	a0,a0
    1e50:	8082                	ret

0000000000001e52 <mkdir>:

int mkdir(const char *path, mode_t mode){
    1e52:	862e                	mv	a2,a1
    1e54:	87aa                	mv	a5,a0
    register long a2 __asm__("a2") = c;
    1e56:	1602                	sll	a2,a2,0x20
    register long a7 __asm__("a7") = n;
    1e58:	02200893          	li	a7,34
    register long a0 __asm__("a0") = a;
    1e5c:	f9c00513          	li	a0,-100
    register long a1 __asm__("a1") = b;
    1e60:	85be                	mv	a1,a5
    register long a2 __asm__("a2") = c;
    1e62:	9201                	srl	a2,a2,0x20
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1e64:	00000073          	ecall
    return syscall(SYS_mkdirat, AT_FDCWD, path, mode);
}
    1e68:	2501                	sext.w	a0,a0
    1e6a:	8082                	ret

0000000000001e6c <getdents>:
    register long a7 __asm__("a7") = n;
    1e6c:	03d00893          	li	a7,61
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1e70:	00000073          	ecall

int getdents(int fd, struct linux_dirent64 *dirp64, unsigned long len){
    //return syscall(SYS_getdents64, fd, dirp64, len);
    return syscall(SYS_getdents64, fd, dirp64, len);
}
    1e74:	2501                	sext.w	a0,a0
    1e76:	8082                	ret

0000000000001e78 <pipe>:
    register long a7 __asm__("a7") = n;
    1e78:	03b00893          	li	a7,59
    register long a1 __asm__("a1") = b;
    1e7c:	4581                	li	a1,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1e7e:	00000073          	ecall

int pipe(int fd[2]){
    return syscall(SYS_pipe2, fd, 0);
}
    1e82:	2501                	sext.w	a0,a0
    1e84:	8082                	ret

0000000000001e86 <dup>:
    register long a7 __asm__("a7") = n;
    1e86:	48dd                	li	a7,23
    __asm_syscall("r"(a7), "0"(a0))
    1e88:	00000073          	ecall

int dup(int fd){
    return syscall(SYS_dup, fd);
}
    1e8c:	2501                	sext.w	a0,a0
    1e8e:	8082                	ret

0000000000001e90 <dup2>:
    register long a7 __asm__("a7") = n;
    1e90:	48e1                	li	a7,24
    register long a2 __asm__("a2") = c;
    1e92:	4601                	li	a2,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2))
    1e94:	00000073          	ecall

int dup2(int old, int new){
    return syscall(SYS_dup3, old, new, 0);
}
    1e98:	2501                	sext.w	a0,a0
    1e9a:	8082                	ret

0000000000001e9c <mount>:
    register long a7 __asm__("a7") = n;
    1e9c:	02800893          	li	a7,40
    __asm_syscall("r"(a7), "0"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(a4))
    1ea0:	00000073          	ecall

int mount(const char *special, const char *dir, const char *fstype, unsigned long flags, const void *data)
{
        return syscall(SYS_mount, special, dir, fstype, flags, data);
}
    1ea4:	2501                	sext.w	a0,a0
    1ea6:	8082                	ret

0000000000001ea8 <umount>:
    register long a7 __asm__("a7") = n;
    1ea8:	02700893          	li	a7,39
    register long a1 __asm__("a1") = b;
    1eac:	4581                	li	a1,0
    __asm_syscall("r"(a7), "0"(a0), "r"(a1))
    1eae:	00000073          	ecall

int umount(const char *special)
{
        return syscall(SYS_umount2, special, 0);
}
    1eb2:	2501                	sext.w	a0,a0
    1eb4:	8082                	ret

0000000000001eb6 <__clone>:

.global __clone
.type  __clone, %function
__clone:
	# Save func and arg to stack
	addi a1, a1, -16
    1eb6:	15c1                	add	a1,a1,-16
	sd a0, 0(a1)
    1eb8:	e188                	sd	a0,0(a1)
	sd a3, 8(a1)
    1eba:	e594                	sd	a3,8(a1)

	# Call SYS_clone
	mv a0, a2
    1ebc:	8532                	mv	a0,a2
	mv a2, a4
    1ebe:	863a                	mv	a2,a4
	mv a3, a5
    1ec0:	86be                	mv	a3,a5
	mv a4, a6
    1ec2:	8742                	mv	a4,a6
	li a7, 220 # SYS_clone
    1ec4:	0dc00893          	li	a7,220
	ecall
    1ec8:	00000073          	ecall

	beqz a0, 1f
    1ecc:	c111                	beqz	a0,1ed0 <__clone+0x1a>
	# Parent
	ret
    1ece:	8082                	ret

	# Child
1:      ld a1, 0(sp)
    1ed0:	6582                	ld	a1,0(sp)
	ld a0, 8(sp)
    1ed2:	6522                	ld	a0,8(sp)
	jalr a1
    1ed4:	9582                	jalr	a1

	# Exit
	li a7, 93 # SYS_exit
    1ed6:	05d00893          	li	a7,93
	ecall
    1eda:	00000073          	ecall
